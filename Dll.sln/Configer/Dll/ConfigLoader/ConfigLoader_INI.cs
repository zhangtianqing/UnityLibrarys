...
{
    public class ConfigLoader_INI : IConfigLoader
    {
        public override A Load<A>()
        {
            A a = new A();

            FieldInfo[] fileInfos = a.GetType().GetFields();
            foreach (FieldInfo fi in fileInfos)
            {
                dynamic obj = fi.FieldType.Assembly.CreateInstance(fi.FieldType.FullName);
                FieldInfo[] fieldInfos = fi.FieldType.GetFields();
                foreach (FieldInfo fi2 in fieldInfos)
                {
                    switch (fi2.FieldType.Name)
                    {
                        case "String":
                            fi2.SetValue(obj, INIHelper.GetString(fi2.Name, fi.Name));
                            break;
                        case "Boolean":
                            fi2.SetValue(obj, INIHelper.GetBool(fi2.Name, fi.Name));
                            break;
                        case "Single":
                            fi2.SetValue(obj, INIHelper.GetFloat(fi2.Name, fi.Name));
                            break;
                        case "Int32":
                            fi2.SetValue(obj, INIHelper.GetInt(fi2.Name, fi.Name));
                            break;
                        case "Double":
                            fi2.SetValue(obj, INIHelper.GetFloat(fi2.Name, fi.Name));
                            break;
        ...
        public override bool Save(object t)
        {
            try
            {
                FieldInfo[] fileInfos = t.GetType().GetFields();
                foreach (FieldInfo fi in fileInfos)
                {
                    var obj = fi.GetValue(t);
                    FieldInfo[] fieldInfos = fi.FieldType.GetFields();
                    foreach (var fi2 in fieldInfos)
                    {
                        switch (fi2.FieldType.Name)
                        {
                            case "String":
                                ;
                                INIHelper.WriteString(fi2.Name, fi.Name, (string)fi2.GetValue(obj));
                                break;
                            case "Boolean":
                                INIHelper.WriteBool(fi2.Name, fi.Name, (bool)fi2.GetValue(obj));
                                break;
                            case "Single":
                                INIHelper.WriteFloat(fi2.Name, fi.Name, (float)fi2.GetValue(obj));
                                break;
                            case "Int32":
                                INIHelper.WriteString(fi2.Name, fi.Name, fi2.GetValue(obj).ToString());
                                break;
                            case "Double":
                                INIHelper.WriteString(fi2.Name, fi.Name, fi2.GetValue(obj).ToString());
                                break;
        ...
        public static class INIHelper
        {
            ...
            public static string mINIFileNameDefault = Application.streamingAssetsPath + "/config.ini";
